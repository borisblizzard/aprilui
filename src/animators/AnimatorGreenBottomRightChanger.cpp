	...
	namespace Animators
	{
		...
		float GreenBottomRightChanger::_getObjectValue() const
		{
			...
			if (imageBox == NULL)
			{
				...
				if (colored == NULL)
				{
					hlog::errorf(logTag, "Animators::GreenBottomRightChanger: parent object '%s' not a subclass of Objects::ImageBox or Objects::Colored!", (this->parent != NULL ? this->parent->getName() : "NULL").cStr());
					return 0.0f;
				}
				return colored->getGreenBottomRight();
			}
		...
		void GreenBottomRightChanger::_setObjectValue(float value)
		{
			...
			if (imageBox == NULL)
			{
				...
				colored->setGreenBottomRight((unsigned char)value);
				return;
			}
			BaseImage* baseImage = imageBox->getImage();
			Image* image = dynamic_cast<Image*>(baseImage);
