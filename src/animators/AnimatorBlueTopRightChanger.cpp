	...
	namespace Animators
	{
		...
		float BlueTopRightChanger::_getObjectValue() const
		{
			...
			if (imageBox == NULL)
			{
				...
				if (colored == NULL)
				{
					hlog::errorf(logTag, "Animators::BlueTopRightChanger: parent object '%s' not a subclass of Objects::ImageBox or Objects::Colored!", (this->parent != NULL ? this->parent->getName() : "NULL").cStr());
					return 0.0f;
				}
				return colored->getBlueTopRight();
			}
		...
		void BlueTopRightChanger::_setObjectValue(float value)
		{
			...
			if (imageBox == NULL)
			{
				...
				colored->setBlueTopRight((unsigned char)value);
				return;
			}
			BaseImage* baseImage = imageBox->getImage();
			Image* image = dynamic_cast<Image*>(baseImage);
