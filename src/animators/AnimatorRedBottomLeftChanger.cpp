	...
	namespace Animators
	{
		...
		float RedBottomLeftChanger::_getObjectValue() const
		{
			...
			if (imageBox == NULL)
			{
				...
				if (colored == NULL)
				{
					hlog::errorf(logTag, "Animators::RedBottomLeftChanger: parent object '%s' not a subclass of Objects::ImageBox or Objects::Colored!", (this->parent != NULL ? this->parent->getName() : "NULL").cStr());
					return 0.0f;
				}
				return colored->getRedBottomLeft();
			}
		...
		void RedBottomLeftChanger::_setObjectValue(float value)
		{
			...
			if (imageBox == NULL)
			{
				...
				colored->setRedBottomLeft((unsigned char)value);
				return;
			}
			BaseImage* baseImage = imageBox->getImage();
			Image* image = dynamic_cast<Image*>(baseImage);
